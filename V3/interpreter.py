...
def write(data):
    with open('/Users/sijin/Desktop/workspace/GTA_Code/dataset/inst.yaml', 'w') as file:
...
def gen_inst(op_info, op_id, fused_array, TR, TC, SR, SC):
    ...
    if op_type == 'gather':
        ...
        if op_order == 'R':
            ...
            outside_times = [1,TC]
    if outside_type != '':
        load_inst.append(gen_load_inst(op_info, op_id, -1, outside_type, TR, TC, SR, SC))
...
def update_inst(full_inst_list,fused_inst_list):
    ...
    for i in range(0,len(full_inst_list)):
        if temp_inst_list[i] != []:
            for j in range(0,len(fused_inst_list[i])):
