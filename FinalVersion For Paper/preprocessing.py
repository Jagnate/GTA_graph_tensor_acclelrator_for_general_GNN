...
def cal_min_sparsity(data_set, tile_size):
    path = "dataset/pubmed/adj_"+data_set+"_"+str(tile_size)+"_1.yaml"
    data = read(path)
    max_tile = 0
    arr = np.array(data)
    x,y = arr.shape
...
if __name__ == '__main__':
    dataset = 'pubmed'
    node_num = 2708
    #row_list = generate_multiples_of_16(16, 32)
    row_list = generate_multiples_of_64(64, 8192)
    path = 'dataset/pubmed/adj_'+dataset+'.npy'
    col = 1
    sp_list = []
    #生成yaml文件
    # for row in tqdm(row_list):
    #     sparsity = calculate_sparsity(row, col, path)
    #     flatten_and_save(sparsity, 'dataset/pubmed/adj_'+dataset+'_'+str(row)+'_'+str(col)+'.yaml')
    #打印非零元最多的稀疏块的稀疏个数
