...
#GTA
isH4 = False
vec_parm = [[8,16],[4,16],[2,16]]
sf_parm = [[8,16],[8,16],[8,16]]
mm_parm = [[8,16],[12,16],[14,16]]
...
def calculate_running_cycle(inst,remain_times,data,node_num,sparse_op_list,sparsity,isSparseFeature,isSparseWeight,isLoadEValid,isLoadNValid,isSinput,fusionBlock_num):
    ...
    if inst['TYPE'] == 'LOAD_E' or inst['TYPE'] == 'STORE_E':
        ...
        if inst['TYPE'] == 'LOAD_W' or inst['TYPE'] == 'LOAD_N':
            ...
            #     s = 0.6 #稀疏特征读取减少60%
        elif (inst['TYPE'] == 'LOAD_N' or inst['TYPE'] == 'STORE_N') and remain_times == 1:
        ...
        if '0_applynode' in inst['ID'] and isSinput: #sinput计算按照加法来进行
            return math.ceil(inst['Tile_Size']*sparsity/hardware_performance.get('VEC_ALU')[fusionBlock_num][0])*math.ceil(inst['Feature_Length']/hardware_performance.get('VEC_ALU')[fusionBlock_num][1])
        s = 1
        # if int(inst['ID'].split('_')[0]) in sparse_op_list and isSparseWeight:
        #     s = 0.5
        res = math.ceil(math.ceil((inst['Feature_Length'])/performance[fusionBlock_num][1])*math.ceil((inst['Weight_Size']/inst['Feature_Length'])/performance[fusionBlock_num][0])*s)
        return res
    else: 
        if 'applyedge' in inst['ID'] or 'gather' in inst['ID']:
            ...
            #     s = 0.65
            res = math.ceil(data[len(data) - remain_times]/performance[fusionBlock_num][0])*math.ceil(inst['Feature_Length']/performance[fusionBlock_num][1])*s
        else:
            res = math.ceil(inst['Tile_Size']/performance[fusionBlock_num][0])*math.ceil(inst['Feature_Length']/performance[fusionBlock_num][1])
        return res
...
def simulate(tile_size_list,dataset,network,layer,isReorder,isSinput, isFlexibleHardware):
        ...
        for inst_num in range(0,len(inst_list)):
            ...
            if state == 'WAITING':
                ...
                if check_dependency(current_inst,inst_num,dep_matrix[fused_list_index],current_next_inst,current_prev_inst) and check_hardware(current_inst, hardware_unit_list, hardware_unit_status):
                    ...
                    #TODO: occpuy_hardware和cal running cycle提起判断如果数据稀疏特殊处理
                    ...
                    inst_status[inst_num]["remain_cycle"] = calculate_running_cycle(current_inst,remain_times,data,node_num,sparse_op_list,sparsity,isSparseFeature,isSparseWeight,isLoadEValid,isLoadNValid, isSinput, current_hardware_num)
...
if __name__ == '__main__':
    ...
    #(5141848, 245730192)
    print(simulate(tile_size_list,data_set,network,layer,isReorder,False,True))