...
def binary_search(isPingpang, tile_size_list, buffer_size, max_tile_size_list, weight_size, edge_buffer_size_per_tile, row_node_buffer_size_per_tile, col_node_buffer_size_per_tile, node_num):
...
def cal_size(op_data, isSinput, isPingpang, inst_fused_dict, fused_block, tile_size_list, buffer_size, max_tile_size_list, node_num, adjust_feature_size=False):
    ...
    #计算分块大小
    tile_row_size, tile_col_size = binary_search(isPingpang, tile_size_list, buffer_size, max_tile_size_list, weight_size, edge_buffer_size_per_tile, row_node_buffer_size_per_tile, col_node_buffer_size_per_tile, node_num)
...
#rw:                总共的访存量
def gen_tile_info(isSinput, isPingpang, op_connected_info, op_num, binary, data, inst_fused_dict, buffer_size, tile_size_list, max_tile_size_list, node_num, adjust_feature_size=False):
    ...
    for fused_block in fused_array:
        per_rw, per_row_size, per_col_size = cal_size(data, isSinput, isPingpang, inst_fused_dict, fused_block, tile_size_list, buffer_size, max_tile_size_list, node_num, adjust_feature_size)
        if per_row_size == -1:
            #print("No Solution")
            return [], [], -1
        rw += per_rw
...
def generate_all_binaries(data, isSinput, isPingpang, breakpoint, op_connected_info, op_num, max_tile_size_list, buffer_size, tile_size_list, bit_length, node_num, skip_bits=None, adjust_feature_size=False):
    ...
    while True:
        ...
        current_fused_array, current_tile_size, current_rw = gen_tile_info(isSinput, isPingpang, op_connected_info, op_num, binary, data, inst_fused_dict, buffer_size, tile_size_list, max_tile_size_list, node_num, adjust_feature_size)
...
#f
#成环
def compile(dataset_name,network_name,layer_name,isReorder,isSinput,isPingpang):
    ...
    res = generate_all_binaries(op_info, isSinput, isPingpang, breakpoint, op_connected_info, op_num, sp_list, buffer_size, tile_size_list, connected_edge_num, node_num, skip_bits)
...
if __name__ == '__main__':
    compile('cora','GCN','layer1',False,False,True)