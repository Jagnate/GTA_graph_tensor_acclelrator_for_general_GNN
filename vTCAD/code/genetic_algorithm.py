...
def eval_sim(data_set, network, isReorder, isSinput, layer, sample, isFlexibleHardware, architectureName):
    ...
    if result:
        ...
        res = simulate(tile_size_list, data_set, network, layer, isReorder, isSinput, isFlexibleHardware, architectureName)
        print(res)
...
def genetic_search(params, data_set, network, isReorder, isSinput, layer,compiler_res, max_it, num_parents, num_fusion_combine, num_tsize_combine, num_fusion_mutate, num_tsize_mutate, max_stable_iter, op_data, inst_fused_dict, isPingpang, buffer_size, breakpoint, isFlexibleHardware, architectureName):
    first_generation = initialize(compiler_res,isPingpang)
    generation = first_generation
    for sample in generation:
        eval_sim(data_set, network, isReorder, isSinput, layer, sample, isFlexibleHardware, architectureName)
    best_sample = generation[0]
...
def genetic_compile(dataset_name,network_name,layer_name,isReorder,isSinput,isPingpang,isFlexibleHardware, architectureName=''):
    max_it = 32     #20，32
    ...
    compiler_res, op_data, buffer_size, node_num, sp_list, tile_size_list = compile(dataset_name,network_name,layer_name,isReorder,isSinput,isPingpang,isFlexibleHardware)
    ...
    res = genetic_search(params, dataset_name, network_name, isReorder, isSinput, layer_name, compiler_res, max_it, num_parents, num_fusion_combine, num_tsize_combine, num_fusion_mutate, num_tsize_mutate, max_stable_iter, op_data, inst_fused_dict, isPingpang, buffer_size, breakpoint, isFlexibleHardware, architectureName)
...
if __name__ == '__main__':
    ...
    res = genetic_compile('cora','GAT','layer3',False,False,True,False, 'OPU')
    #GTA
    #Final Sample:  {'fusion': [[0, 1, 2], [4, 5, 6, 7, 8], [3, 9, 10, 11, 12, 13]], 'tsize': [[1808, 1], [2720, 1], [2720, 1]], 'pingpong': True, 'latency': 81660, 'mem_access': 4192512, 'pattern': '1100011111001111'}
