...
def binary_search(isPingpang, tile_size_list, buffer_size, max_tile_size_list, weight_size, edge_buffer_size_per_tile, row_node_buffer_size_per_tile, col_node_buffer_size_per_tile, node_num, flexibleBuffer = False):
    ...
    w_buffer = math.floor(buffer_size * 0.75)
    e_buffer = math.floor(buffer_size * 0.5)
    n_buffer = math.floor(buffer_size * 0.5)
    ...
    while left < right:
        ...
        if flexibleBuffer:
            if weight_size < w_buffer and node_buffer_size < n_buffer and edge_buffer_size < e_buffer:
                left = mid
            else:
                right = mid - 1
        else:
...
if __name__ == '__main__':
    compile('cora','GAT','layer1',False,False,True,False)